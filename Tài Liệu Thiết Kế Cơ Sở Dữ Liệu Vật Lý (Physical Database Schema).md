...
_Bản vẽ thiết kế sơ đồ lưới 2D của kho bãi._
...
- `width` / `height` (INT, DEFAULT 100): Kích thước chiều rộng và chiều cao của lưới bản vẽ tổng thể (giới hạn biên lưới).
...
_Phân rã sơ đồ Layout thành các Khu vực mặt sàn cụ thể (Cấp độ 1)._

- `id` (UUID, PK): Mã khu vực.
    
- `layout_id` (UUID, FK $\rightarrow$ `Warehouse_Layout.id`, NOT NULL): Thuộc về bản vẽ sơ đồ nào.
    
- `name` (VARCHAR(100), NOT NULL): Tên khu vực (Ví dụ: Khu A, Khu Hàng Dễ Vỡ).
    
- `code` (VARCHAR(50), NOT NULL): Mã định danh khu vực (Ví dụ: `ZONE_A`).
    
- `max_weight` (DECIMAL(10,2)): Tải trọng tối đa khu vực chịu được (kg) - Dùng để kiểm tra sức chứa.
    
- `max_volume` (DECIMAL(10,2)): Thể tích không gian tối đa của khu vực (m3) - Dùng để kiểm tra sức chứa.
    
- `coordinate_x` / `coordinate_y` (INT): Tọa độ góc trên bên trái ($x, y$) tuyệt đối trên lưới đồ họa 2D của Layout.
    
- `width` / `height` (INT): Kích thước chiều rộng và chiều dài ($w, h$) ô lưới của khu vực trên giao diện.
...
_Hạ tầng các Kệ hàng đặt nằm bên trong một Zone (Cấp độ 2)._

- `id` (UUID, PK): Mã kệ hàng.
    
- `zone_id` (UUID, FK $\rightarrow$ `Warehouse_Zone.id`, NOT NULL): Kệ nằm trong khu vực nào.
...
- `coordinate_x` / `coordinate_y` / `width` / `height` (INT): Tọa độ tương đối ($x, y$) so với Zone chứa nó và kích thước kéo thả của kệ hàng.
...
_Chi tiết các Ô chứa nhỏ/Tầng kệ nằm trên một Rack (Cấp độ 3 - Cấp độ lưu trữ hàng sâu nhất)._
...
- `coordinate_x` / `coordinate_y` / `width` / `height` (INT): Tọa độ tương đối ($x, y$) so với Rack chứa nó (đại diện cho dòng/cột trên mặt kệ) và kích thước ô chứa.
...
- `zone_id` (UUID, FK $\rightarrow$ `Warehouse_Zone.id`, NULL): Mã định vị Khu vực nếu hàng để thẳng dưới đất (Không dùng kệ).
...
- `zone_id` (UUID, FK $\rightarrow$ `Warehouse_Zone.id`, NOT NULL): Vị trí khu vực chỉ định (bắt buộc chọn khi nhập/xuất để quản lý vị trí chính xác).
