...
_Định nghĩa các gói dịch vụ WMS dành cho Tenant do nền tảng cung cấp._

- `id` (UUID, PK): Mã gói dịch vụ.
    
- `name` (VARCHAR(100), NOT NULL): Tên gói (Ví dụ: Gói Basic, Gói Pro, Gói Enterprise).
    
- `features` (TEXT): Lưu chuỗi JSON mô tả danh sách tính năng hiển thị cho FE render bảng so sánh.
    
- `price` (DECIMAL(15,2), DEFAULT 0.00): Giá gói dịch vụ (VNĐ).
    
- `duration_days` (INT, NOT NULL, DEFAULT 30): Thời hạn sử dụng của gói (tính theo ngày).

- `max_staff` (INT, NOT NULL, DEFAULT 0): Số lượng nhân viên kho tối đa cho phép có (0 = không giới hạn).
...
_Quản lý thời hạn, trạng thái sử dụng gói dịch vụ WMS và lưu vết ảnh chụp (Snapshot) thông số gói của Tenant tại thời điểm mua._

- `id` (UUID, PK): Mã lượt đăng ký gói.
    
- `tenant_id` (UUID, FK $\rightarrow$ `User.id`, NOT NULL): Tenant mua gói dịch vụ.
    
- `package_id` (UUID, FK $\rightarrow$ `Service_Package.id`, NOT NULL): Gói dịch vụ gốc được chọn mua.
    
- `start_date` (TIMESTAMP, NOT NULL): Ngày bắt đầu kích hoạt gói.
    
- `end_date` (TIMESTAMP, NOT NULL): Ngày hết hạn gói dịch vụ.
    
- `status` (VARCHAR(50), DEFAULT 'ACTIVE'): Trạng thái sử dụng (`ACTIVE`, `EXPIRED`, `CANCELLED`, `SUPERSEDED` - Đã ngắt sớm do nâng cấp gói mới).

- `snapshot_max_staff` (INT, NOT NULL, DEFAULT 0): Quota Staff tại thời điểm mua (Snapshot giữ nguyên quyền lợi nếu Admin sửa gói sau này).

- `snapshot_price` (DECIMAL(15,2)): Giá mua gói tại thời điểm mua (Snapshot).

- `snapshot_features` (TEXT): Chuỗi JSON mô tả tính năng tại thời điểm mua (Snapshot).

- `snapshot_package_name` (VARCHAR(150)): Tên gói tại thời điểm mua (Snapshot).
    
- `created_at` / `updated_at` (TIMESTAMP): Thời gian mua và cập nhật gói.
    
- **Indexes:** `tenant_id` (Kiểm tra nhanh xem Tenant hiện tại có gói dịch vụ hợp lệ không), `status`.
