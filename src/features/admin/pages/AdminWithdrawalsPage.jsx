...
const RejectModal = ({ withdrawal, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await dispatch(
      rejectWithdrawal({ id: withdrawal.id, adminNotes: adminNotes.trim() })
    )
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Withdrawal info */}
        <div className="mb-5 space-y-1 rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm">
          <p>
            <span className="text-slate-500">Bank:</span>{' '}
            <span className="font-medium text-slate-800">{withdrawal.bankName || '—'}</span>
          </p>
          <p>
      ...
      >
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
            ...
            <Badge
              variant={STATUS_CONFIG[withdrawal.status]?.variant || 'slate'}
              size="sm"
