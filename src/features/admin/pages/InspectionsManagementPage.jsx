...
const AssignModal = ({ inspection, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await dispatch(
      assignInspector({ id: inspection.id, inspectorId: inspectorId.trim() })
    )
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              Select the user with the Inspector role in the system to assign
            </p>
          </div>
          ...
          {inspection.images && inspection.images.length > 0 && (
            <div className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
              <p className="mb-2 flex items-center gap-1 text-xs text-slate-400">
                <ImageIcon size={11} /> Photo ({inspection.images.length})
              </p>
          ...
          <main className="mx-auto w-full max-w-400 space-y-6 p-6 md:p-8">
            ...
            <div className="flex flex-col items-center justify-between gap-4 rounded-xl border border-slate-200 bg-white p-4 md:flex-row">
              <div className="relative w-full md:w-96">
                ...
                <input
                  type="text"
                  placeholder="Search by ID, warehouse name, address, inspector..."
                  value={searchText}
