...
const EditConfigModal = ({ configItem, packages, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        if (!configValue.trim()) { setLocalError("Please enter a value."); return }
        setLocalError(null)
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="mb-4 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm">
                        <p className="text-xs text-blue-500 font-medium mb-1">Setup: {title}</p>
                        <p className="text-slate-600 text-xs">{configItem.description}</p>
                    </div>

                    <div>
                        <label className="mb-1.5 block text-sm font-semibold text-slate-700">Configuration value <span className="text-rose-500">*</span></label>
                        {configItem.configKey === 'warehouse_publish_package_id' ? (
                            ...
                            >
                                ...
                                {packages.map(p => (
                                    <option key={p.id} value={p.id}>{p.name} - {Number(p.price).toLocaleString('en-US')} ₫</option>
                                ))}
                            </select>
                        ) : (
                        ...
                        <p className="mt-1 text-xs text-slate-400">
                            ...
                            {configItem.configKey === 'warehouse_publish_package_id' && "Select the default warehouse posting package"}
                        </p>
                    </div>
                    ...
                    <main className="mx-auto w-full max-w-400 space-y-6 p-6 md:p-8">
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                            {loading ? (
                                ...
                                <div className="py-20 text-center text-sm text-slate-400">There are no configurations yet.</div>
                            ) : (
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm">
                                        <thead>
                                            <tr className="border-b border-slate-100 bg-slate-50">
                                                {["Configuration Name / Key", "Value", "Description", "Update", ''].map((h) => (
                                                ...
                                                return (
                                                    <motion.tr key={cfg.configKey} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                                                        ...
                                                        <td className="px-5 py-4 text-slate-600 max-w-sm">
                                                            <p className="line-clamp-2" title={cfg.description}>{cfg.description || "No description available"}</p>
                                                        </td>
