...
const WarehouseTypeFormModal = ({ item, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (updateWarehouseType.fulfilled.match(result) || createWarehouseType.fulfilled.match(result)) {
            onClose()
        }
    }
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                ...
                <div className="mb-5 flex items-start justify-between border-b border-slate-100 pb-4">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">{isEdit ? 'Cập nhật Loại Kho' : 'Thêm Loại Kho Mới'}</h2>
                        {isEdit && <p className="mt-0.5 text-xs text-slate-500 font-mono">ID: {shortId(item.id)}</p>}
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="mb-1.5 block text-sm font-semibold text-slate-700">Tên Loại Kho <span className="text-rose-500">*</span></label>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                ...
                <p className="mt-2 text-sm text-slate-500">
                    Bạn có chắc chắn muốn xóa loại kho <strong className="text-slate-800">{item.name}</strong> không? Hành động này không thể hoàn tác.
                </p>

                {actionError && <p className="mt-4 rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-600 text-left flex items-center gap-1.5"><AlertCircle size={14}/> {actionError}</p>}
...
const WarehousesTypePage = () => {
    ...
    } = useSelector((s) => s.adminWarehouseType)
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
                    ...
                    <main className="mx-auto w-full max-w-5xl space-y-6 p-6 md:p-8">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full min-w-[240px] rounded-xl border border-slate-200 bg-white py-2.5 pl-9 pr-4 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 shadow-sm"
                                    />
                                </div>
                                <button onClick={() => setShowCreate(true)} className="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 shadow-sm whitespace-nowrap">
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                            {loading ? (
                                ...
                                <div className="py-20 text-center text-sm text-slate-400">Không tìm thấy loại kho nào trong hệ thống.</div>
                            ) : (
                                            ...
                                            {data.map((item) => (
                                                <motion.tr key={item.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                                                    ...
                                                    <td className="px-5 py-4 w-1/4">
                                                        ...
                                                        <p className="font-mono text-[10px] text-slate-400 mt-1 inline-block bg-slate-100 px-1.5 py-0.5 rounded">{shortId(item.id)}</p>
                                                    </td>
                                                    <td className="px-5 py-4">
                                                        <p className="text-slate-600 leading-relaxed max-w-2xl">{item.description || <span className="text-slate-400 italic">Không có mô tả</span>}</p>
                                                    </td>
                                                    <td className="px-5 py-4 text-right w-32">
                                                        <div className="inline-flex items-center gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                                                            <button onClick={() => setEditingItem(item)} className="rounded-lg p-2 text-blue-600 hover:bg-blue-50 transition-colors" title="Chỉnh sửa">
                                                                <Edit3 size={18} />
                                                            </button>
                                                            <button onClick={() => setDeletingItem(item)} className="rounded-lg p-2 text-rose-600 hover:bg-rose-50 transition-colors" title="Xóa">
                                                                <Trash2 size={18} />
                                                            </button>
                                                        </div>
                                                    </td>
                            ...
                            {totalPages > 1 && (
                                <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-bold text-slate-700 px-2">{page + 1}</span>
            ...
            <AnimatePresence>
                ...
                {editingItem && <WarehouseTypeFormModal key="edit" item={editingItem} onClose={() => setEditingItem(null)} />}
                {deletingItem && <DeleteConfirmModal key="delete" item={deletingItem} onClose={() => setDeletingItem(null)} />}
            </AnimatePresence>
        </div>
    )
}
